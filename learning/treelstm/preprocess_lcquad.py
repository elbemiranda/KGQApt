...
from parser_qasparql.lc_quad_linked import LC_Qaud_LinkedParser
...
def query_parse(filepath):
    ...
    with open(filepath, encoding='utf-8') as datafile, \
            open(tokpath, 'w', encoding='utf-8') as tokfile, \
            open(parentpath, 'w', encoding='utf-8') as parentfile:
...
def generalize_question(a, b, parser=None):
    ...
    # TODO: Adicionar menções de entidades através do NER
...
def split(data, parser=None):
    if isinstance(data, str):
        with open(data, encoding='utf-8') as datafile:
    ...
    for item in tqdm(dataset):
        ...
        for query in item["generated_queries"]:
            ...
            id_list.append(i + '\n')
            a_list.append(a.encode('utf-8', 'ignore').decode('utf-8') + '\n')
            b_list.append(b.encode('utf-8', 'ignore').decode('utf-8') + '\n')
            sim_list.append(sim + '\n')
    return a_list, b_list, id_list, sim_list


def save_split(dst_dir, a_list, b_list, id_list, sim_list):
    with open(os.path.join(dst_dir, 'a.txt'), 'w', encoding='utf-8') as afile, \
            open(os.path.join(dst_dir, 'b.txt'), 'w', encoding='utf-8') as bfile, \
            open(os.path.join(dst_dir, 'id.txt'), 'w', encoding='utf-8') as idfile, \
            open(os.path.join(dst_dir, 'sim.txt'), 'w', encoding='utf-8') as simfile:
...
if __name__ == '__main__':
    ...
    ds = json.load(open("output/lcquad_gold.json", encoding = "utf-8"))
