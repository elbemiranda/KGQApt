...
if __name__ == "__main__":
    ...
    ds = LC_Qaud(path="./data/LC-QUAD/data_pt.json")
    tmp = []
    for qapair in prepare_dataset(ds).qapairs:
        ...
        results_count = 0
        if "results" in raw_row["answers"].keys():
            results_count = (len(raw_row["answers"]["results"]["bindings"]))
        
        print(f'{results_count}\t{raw_row["id"]}')
        tmp.append(raw_row)
