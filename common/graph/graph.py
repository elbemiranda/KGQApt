...
class Graph:
    ...
    def __parallel_one_hop_graph(self, params):
        entity_uri = params[0]
        relation_uri = params[1]
            ...
            for relation_uri in relation_item.top_uris(top_uri):
                    for entity_uris in itertools.product(*[items.top_uris(top_uri) for items in entity_items]):
                        for entity_uri in itertools.combinations(entity_uris, number_of_entities):
                            #pbar.update(1)
                            self.__parallel_one_hop_graph((entity_uri,relation_uri))
                            #params.append((entity_uri,relation_uri))
        #result = p_map(self.__parallel_one_hop_graph, params)
