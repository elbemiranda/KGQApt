...
from learning.classifier.nnclassifier import NNClassifier
from learning.classifier.rfclassifier import RFClassifier
from learning.classifier.svmclassifier import SVMClassifier
from parser_qasparql.lc_quad import LC_Qaud
...
from parser_qasparql.lc_quad import LC_QaudParser
import common.utility.utility as utility
import ujson
import learning.treelstm.Constants as Constants
import numpy as np
import itertools
from parser_qasparql.lc_quad_linked import LC_Qaud_Linked
from parser_qasparql.qald import Qald
...
class Orchestrator:
    def __init__(self, logger, question_classifier, double_relation_classifer, parser, filepath, auto_train=True):
        ...
        self.question_classifier_dataset_ready = False
        self.double_relation_classifier_dataset_ready = False
    ...
    def prepare_question_classifier_dataset(self, file_path=None):
        print('preparando dataset do question classifier')
        if file_path is None:
            ds = LC_Qaud()
            self.filepath = ds.get_dataset()
        else:
        ...
        for qapair in ds.qapairs:
            ...
            y.append(question_type)

            question_type_dataset.append({'X': qapair.question.text, 'y': question_type})
        with open(self.filepath.replace('.json', '-question-type-dataset.json'), mode= "w", encoding="utf-8") as dataset_file:
            dataset_file.write(json.dumps(question_type_dataset))
            self.question_classifier_dataset_ready = True

    def get_classifier_dataset(self, file_path):
        with open(file_path, mode = 'r', encoding= "utf-8") as dataset_file:
    ...
    def prepare_double_relation_classifier_dataset(self, file_path=None):
        print('preparando dataset do double relation classifier')
        if file_path is None:
            ds = LC_Qaud()
            self.filepath = ds.get_dataset()
        else:
        ...
        for qapair in ds.qapairs:
            ...
            y.append(double_relation)
            double_relation_dataset.append({'X': qapair.question.text, 'y': double_relation})                

        with open(self.filepath.replace('.json', '-double-relation-dataset.json'), mode= "w", encoding="utf-8") as dataset_file:
            dataset_file.write(json.dumps(double_relation_dataset))
            self.double_relation_classifier_dataset_ready = True

    def train_question_classifier(self, file_path=None, test_size=0.2):
        print('Treinando question classifier')
        #X, y = self.prepare_question_classifier_dataset(self.filepath)
        if not self.question_classifier_dataset_ready:
            self.prepare_question_classifier_dataset(self.filepath)        
        X, y = self.get_classifier_dataset(self.filepath.replace('.json', '-question-type-dataset.json'))
    ...
    def train_double_relation_classifier(self, file_path=None, test_size=0.2):
        print('Treinando double relation classifier')
        #X, y = self.prepare_double_relation_classifier_dataset(self.filepath)
        if not self.double_relation_classifier_dataset_ready:
            self.prepare_double_relation_classifier_dataset(self.filepath)        
        X, y = self.get_classifier_dataset(self.filepath.replace('.json', '-double-relation-dataset.json'))
        self.X_train, self.X_test, self.y_train, self.y_test = train_test_split(X, y, test_size=test_size,
                                                                                random_state=42)
    ...
    def generate_query(self, question, entities, relations, h1_threshold=9999999, question_type=None):
        ...
        args.expname = "lc_quad,epoch=15,train_loss=0.09359747171401978"
        args.mem_dim = 150
        args.hidden_dim = 50
        args.num_classes = 2
        args.input_dim = 300
            ...
            for idx in range(len(sorted_queries)):
                ...
                if answerset == qapair.answerset:
                    ...
                    if answerset == qapair.answerset:
                        ...
                        precision= intersect/len(answerset) if len(answerset) > 0 else 0
...
if __name__ == "__main__":
    ...
    if not os.path.isfile("./data/LC-QUAD/linked_test.json"):
        ...
        with open('data/LC-QUAD/linked_answer.json', 'r', encoding='utf-8') as f:
            data = json.load(f)
        ...
        for i in data:
            if i['id'] in test_id:
                test_data.append(i)
        with open('data/LC-QUAD/linked_test.json', 'w', encoding='utf-8') as f:
    ...
    #question_type_classifier_svm = SVMClassifier(os.path.join(question_type_classifier_path, "svm_pt_embedding.model"))
    #double_relation_classifier_svm = SVMClassifier(os.path.join(double_relation_classifier_path, "svm_pt_embedding.model"))

    question_type_classifier_rf = RFClassifier(os.path.join(question_type_classifier_path, "rf_pt_tfidf.model"))
    #double_relation_classifier_rf = RFClassifier(os.path.join(double_relation_classifier_path, "rf_pt_embedding.model"))

    #question_type_classifier_nn = NNClassifier(os.path.join(question_type_classifier_path, "nn_pt_embedding.model"))
    double_relation_classifier_nn = NNClassifier(os.path.join(double_relation_classifier_path, "nn_pt_embedding.model"))    
    ...
    o = Orchestrator(logger, question_type_classifier_rf, double_relation_classifier_nn, parser, None, False)

    #o.question_classifier = question_type_classifier_svm
    #o.train_question_classifier()
    #o.question_classifier = question_type_classifier_rf
    #o.train_question_classifier()
    #o.question_classifier = question_type_classifier_nn
    #o.train_question_classifier()

    #o.double_relation_classifer = double_relation_classifier_nn
    #scores = o.train_double_relation_classifier()
    #print(scores)

    #y_pred = o.double_relation_classifer.predict(o.X_test)
    #print(accuracy_score(o.y_test, y_pred))
    #print(classification_report(o.y_test, y_pred, digits=3))    
    

    linker = Earl(path="data/LC-QUAD/entity_lcquad_test-without_ptrl.json")
    ...
    for qapair in ds.qapairs:
        ...
        if qapair.answerset is None or len(qapair.answerset) == 0:
            ...
            result, where, question_type, type_confidence, precision, recall = o.sort_query(linker, ds.parser.kb, ds.parser, qapair, question_type_classifier_rf, True)
            stats.inc(result)
        ...
        print(f"{stats['total']} of {qnt}")
        if stats["total"] % 100 == 0:
            with open("output/{}-without_ptrl.json".format(output_file), "w") as data_file:
