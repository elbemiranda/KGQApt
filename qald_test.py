...
class Orchestrator:
    def __init__(self, logger, question_classifier, double_relation_classifer, parser, filepath, auto_train=True):
        ...
        self.dep_tree_cache_file_path = './caches/dep_tree_cache_lcquad.json'
    ...
    def rank(self, args, question_id, question, generated_queries):
        if len(generated_queries) == 0:
            return []
        if 2 > 1:
            try:
                # Load the model
                checkpoint_filename = '%s' % os.path.join(args.save, args.expname)
                ...
                output_dir = f"./output/lcquadall/{question_id}"
                preprocess_lcquad.make_dirs([output_dir])
                preprocess_lcquad.save_split(output_dir, *preprocess_lcquad.split(json_data, self.parser))

                if False:#if question in self.dep_tree_cache:
    ...
    def generate_query(self, question_id, question, entities, relations, h1_threshold=9999999, question_type=None):
        ask_query = False
        sort_query = False
        count_query = False
        ...
        args.expname = "lc_quad,epoch=15,train_loss=0.09359747171401978.pt"
        args.mem_dim = 150
        args.hidden_dim = 50
        args.num_classes = 2
        args.input_dim = 300
        ...
        # args.cuda = True
        try:
            scores = self.rank(args, question_id, question, valid_walks)
    ...
    def sort_query(self, linker, kb, parser, qapair, question_type_classifier, force_gold=True):
        ...
        if (len(answerset_true) == 0):
            status, raw_answer_true = kb.query(qapair.sparql.query.replace("http://dbpedia.org/ontology/", "http://dbpedia.org/property/"))
            answerset_true = AnswerSet(raw_answer_true, parser.parse_queryresult)        
        ...
        if qapair.answerset is None or len(qapair.answerset) == 0:
            ...
            for comb in combination_list:
                if len(generated_queries) == 0:
                    generated_queries, question_type, type_confidence = self.generate_query(qapair.id, question, comb[0], comb[1])
                    if len(generated_queries) > 0:
                        ask_query = False
                        count_query = False
...
if __name__ == "__main__":
    def question_answer(qapair):
        ...
        with open(f"output/lcquadall/experiments/{expriment}/{qapair.id}.json", "w") as data_file:
            json.dump(output_row, data_file, sort_keys=True, indent=4, separators=(',', ': '), cls=NumpyEncoder)
    ...
    expriment = 'with_ptrl'
    #output_file = f"qaldanswer_output-{expriment}"
    output_file = f"lcquadanswer_output-{expriment}"
    #linker = Earl(path=f"data/QALD/entity_qald-{expriment}.json")
    linker = Earl(path=f"data/LC-QUAD/entity_lcquad-{expriment}.json")
    ...
    o = Orchestrator(logger, question_type_classifier_rf, double_relation_classifier_nn, parser, question_type_classifier_path, True)

    tmp = []
    output = []
    qnt = len(ds.qapairs)
    
    #files_ids_processed = glob(f"output/qald/experiments/{expriment}/*.json")
    files_ids_processed = glob(f"output/lcquadall/experiments/{expriment}/*.json")
    ids_processed = [file.split('/')[-1].replace('.json','') for file in files_ids_processed]
    qapairs = [qapair for qapair in ds.qapairs if not qapair.id in ids_processed]

    for file in files_ids_processed:
        with open(file, "r", encoding="utf-8") as data_file:
            file_processed = json.load(data_file)
            output.append(file_processed) 
    
    output_parallel = p_map(question_answer, qapairs)
    output = output + output_parallel     
...
#    output = None
#    with open('output/qaldanswer_output-with_ptrl.json', 'r', encoding="utf-8") as data_file:
...
#             print(f"{stats['total']} of {qnt}")
#             if stats["total"] % 10 == 0:
#                 with open("output/{}-with_ptrl.json".format(output_file), "w") as data_file:
#                     json.dump(output, data_file, sort_keys=True, indent=4, separators=(',', ': '), cls=NumpyEncoder)
